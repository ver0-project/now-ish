...
describe('millisecond (ms)', () => {
    const unit = units.get('ms') as Unit;
    ...
    describe('round-down', () => {
        const cases: RoundCase[] = [
            {name: 'truncate nanoseconds at start', input: '2024-03-15T10:30:00.500000001', expected: '2024-03-15T10:30:00.5'},
...
describe('second (s)', () => {
    const unit = units.get('s') as Unit;
    const ctx = {timezone: 'UTC'};
...
describe('minute (m)', () => {
    const unit = units.get('m') as Unit;
    const ctx = {timezone: 'UTC'};
...
describe('hour (h)', () => {
    const unit = units.get('h') as Unit;
    const ctx = {timezone: 'UTC'};
...
describe('day (d)', () => {
    const unit = units.get('d') as Unit;
    const ctx = {timezone: 'UTC'};
...
describe('week (w)', () => {
    const unit = units.get('w') as Unit;
    const ctx = {timezone: 'UTC'};
...
describe('month (mo)', () => {
    const unit = units.get('mo') as Unit;
    const ctx = {timezone: 'UTC'};
...
describe('year (y)', () => {
    const unit = units.get('y') as Unit;
    const ctx = {timezone: 'UTC'};

    describe('add', () => {
        const cases: AddCase[] = [
            {name: 'add one year', input: '2024-03-15T10:30:00', amount: 1, expected: '2025-03-15T10:30:00'},
            {name: 'leap to non-leap clamps Feb 29', input: '2024-02-29T10:00:00', amount: 1, expected: '2025-02-28T10:00:00'},
...
describe('now', () => {
    ...
    it('passes timezone to Temporal.Now.zonedDateTimeISO', () => {
        const fixedTime = Temporal.ZonedDateTime.from(
            '2024-03-15T15:30:00[Europe/Moscow]'
        );
        vi.spyOn(Temporal.Now, 'zonedDateTimeISO').mockReturnValue(fixedTime);
